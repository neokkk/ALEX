...
long long memory_consumption(alex::Alex<KEY_TYPE, PAYLOAD_TYPE> &index) {
  long long memory = -1;
  memory += index.model_size();
  memory += index.data_size();
  return memory;
}
...
int main(int argc, char* argv[]) {
  ...
  auto init_num_keys = stol(get_required(flags, "init_num_keys"));
  auto total_num_keys = stol(get_required(flags, "total_num_keys"));
  auto batch_size = stol(get_required(flags, "batch_size"));
  auto index_size = stoll(get_with_default(flags, "index_size", "-1"));
  ...
  long num_inserts_per_batch = static_cast<long>(batch_size * insert_frac);
  long num_lookups_per_batch = batch_size - num_inserts_per_batch;
  ...
  while (true) {
    ...
    // Do inserts
    long num_actual_inserts = std::min(num_inserts_per_batch, total_num_keys - i);
    long num_keys_after_batch = i + num_actual_inserts;
    std::cout << "num_actual_inserts: " << num_actual_inserts << std::endl;
    std::cout << "num_keys_after_batch: " << num_keys_after_batch << std::endl;
